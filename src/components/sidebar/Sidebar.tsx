...
export default function Sidebar() {
  ...
  return (
    <motion.aside
      ...
      className={cn(
        "relative flex h-screen shrink-0 flex-col overflow-hidden border-r border-gray-200 bg-[#F7F7F5] dark:border-[#2f2f2f] dark:bg-[#252525]"
      )}
    >
      <div className="flex h-full w-[260px] flex-col">
        ...
        <div className="px-3 py-1 space-y-0.5">
          ...
          <button
            onClick={() => setCurrentPage("__habits__")}
            className={cn(
              "flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-sm transition-colors",
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 space-y-4">
          ...
          {favorites.length > 0 && (
            <div>
              ...
              {favorites.map((p) => (
                <PageTreeItem key={p.id} pageId={p.id} />
              ))}
            </div>
          )}
          ...
          {recentPages.length > 0 && (
            <div>
              ...
              {recentPages.map((p) => (
                <PageTreeItem key={`recent-${p.id}`} pageId={p.id} />
              ))}
            </div>
          )}
